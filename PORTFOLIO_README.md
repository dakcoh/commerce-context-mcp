...
> Java Spring 기반 이커머스 웹 백엔드를 개발할 때 필요한 유통 도메인 지식과 구현 가이드를 주입하는 **공용 MCP 지식 서버**
...
| 도구 | 해결하는 문제 |
|------|-------------|
| `get_commerce_foundation_context` | 신규 쇼핑몰 구축 시 상품·가격·주문·재고·결제·물류·유통·정산·운영 기반 지식 제공 |
| `get_commerce_foundation_checklist` | 전체 아키텍처와 출시 전 누락 사항 검토 |
| `search_commerce_knowledge` | 여러 도메인에 걸친 범용 이커머스 지식 통합 검색 |

### Java Spring 이커머스 구현 지식 (3개)

| 도구 | 해결하는 문제 |
|------|-------------|
| `get_spring_commerce_implementation_context` | Spring Boot 쇼핑몰 구축 시 트랜잭션·JPA·Redis·Outbox·배치·테스트 기준 제공 |
| `get_spring_commerce_checklist` | Java Spring 이커머스 출시 전 구현 누락 검토 |
| `search_spring_commerce_knowledge` | `@Transactional`, JPA, Redis, Outbox, Testcontainers 등 Spring 구현 지식 검색 |
...
│  inventory.yml / payment.yml / settlement.yml         │
│  coupon.yml / commerce.yml / spring-commerce.yml      │
└─────────────────────────────────────────────────────┘
```
...
SettlementContextToolTest           6개  — 정산 도구 응답 검증
CouponContextToolTest               6개  — 쿠폰 도구 응답 검증
CommerceKnowledgeServiceTest        7개  — 범용 정규화 지식 검증
CommerceContextToolTest             3개  — 범용 이커머스 도구 응답 검증
SpringCommerceKnowledgeServiceTest  7개  — Java Spring 구현 지식 검증
