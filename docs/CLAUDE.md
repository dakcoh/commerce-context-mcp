...
일주일 내 서비스 배포 전에는 core 모듈 분리보다 `npx commerce-context-mcp`, `--help`, `doctor`,
Claude/Cursor 설정 복붙 같은 외부 사용자 진입점을 먼저 고도화한다.
상세 계획은 `docs/EXTERNAL_USAGE_PLAN.md`를 따른다.
...
# 3. 태그 푸시 → GitHub Actions가 npm 자동 배포
