...
따라서 core 모듈 분리는 당장 배포 전 필수 작업이 아니라 장기 고도화 계획으로 둔다.

현재 우선 진행 문서는 `docs/EXTERNAL_USAGE_PLAN.md`다.
...
일주일 내 서비스 배포를 고려하면 당장 전체 구조를 크게 바꾸기보다, 외부 사용자가 `npx`와 MCP 설정만으로 쉽게 쓰게 만드는 작업을 먼저 진행한다.
core 모듈 분리는 배포 안정화 후 구조화 API가 필요해지는 시점에 적용한다.
...
일주일 내 배포가 목표라면 Gradle 멀티 모듈까지 한 번에 진행하지 않는다.
먼저 패키지 경계와 구조화 API만 만들고, 배포 안정화 후 멀티 모듈로 분리한다.
...
배포 전 고도화 목표라면 **Gradle 멀티 모듈은 미루고, core 패키지 + 구조화 API부터 도입**하는 것이 가장 안전하다.
